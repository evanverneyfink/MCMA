...
namespace Mcma.Core
{
    public static class StringExtensions
    {
        ...
        {
            return toSplit.Split(new[] {splitOn}, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).ToArray();
        }
        ...
        {
            return source.SplitOn(pairDelimiter).Select(x => x.ToKeyValuePair(valueDelimiter)).ToDictionary(x => x.Item1, x => x.Item2);
        }
        ...
        public static T Parse<T>(this string textValue)
        {
            return (T)textValue.Parse(typeof(T));
        }
        ...
        public static T TryParse<T>(this string textValue, T defaultValue)
        {
            return textValue.TryParse(out T tmp) ? tmp : defaultValue;
        }
