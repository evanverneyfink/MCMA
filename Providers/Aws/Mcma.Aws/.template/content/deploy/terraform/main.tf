...
resource "aws_api_gateway_integration" "mcma_service_api_method-integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${var.region}:${var.account_id}:function:${aws_lambda_function.api_mcma_service_lambda.function_name}/invocations"
...
resource "aws_lambda_permission" "apigw_lambda" {
  ...
  source_arn = "arn:aws:execute-api:${var.region}:${var.account_id}:${aws_api_gateway_rest_api.mcma_service_api.id}/*/${aws_api_gateway_method.mcma_service_api_method.http_method}/*"
}
...
output "lambda_arn" {
  value = "${aws_lambda_function.api_mcma_service_lambda.arn}"
}
